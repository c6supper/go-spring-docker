...
cat go-spring.template.yaml | DOLLAR='$' envsubst > docker-compose.yml
...
docker stop `docker ps -aq -f name=go-spring_*`
docker rm `docker ps -aq -f name=go-spring_*`
docker network prune -f
docker-compose -p "go-spring" up -d